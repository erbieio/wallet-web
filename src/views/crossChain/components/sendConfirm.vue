  ...
  export default defineComponent({
    ...
    setup(props: any, context: SetupContext) {
      ...
      watch(
        ...
        async (n) => {
          if (!n) {
            ...
            const { address, gasPrice, gasLimit } = props.data;
            if (!address) {
              try {
                const num = gasLimit;
      ...
      const handleComfirm = async () => {
        ...
        // PolygonChainContractAddr2
        const amountWei = currentNetwork.value.isMain ? '0' : (utils.parseEther(props.data.amount?.toString() || '0').toString())
        const val = currentNetwork.value.isMain ? utils.parseEther(props.data.amount?.toString() || '0') : utils.parseEther('0')
        try {
          txData = await store.dispatch("account/contractTransaction",{
            contractAddr: tokenContractAddress,
            functionName: 'crossOut',
            abi,
            args:[props.data.to, amountWei, {
                value: val
            }]
          });
          ...
          if (receipt.status) {
            $tradeConfirm.update({ status: "success", hash: txData.hash, callBack });
          } else {
            $tradeConfirm.update({ status: "fail", hash: txData.hash, callBack });
          }
        } catch (err: any) {
            console.error(err)
